...
<Tabs items={['npm', 'pnpm', 'yarn']}>
  ...
  <Tabs.Tab>
    ```bash
    pnpm init && pnpm add -D typescript @types/node ts-node && pnpm exec tsc --init && mkdir src && echo 'async function example() { console.log("Running example!")}; example()' > src/quickstart.ts
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```bash
...
<Tabs items={['npm', 'pnpm', 'yarn']}>
  ...
  <Tabs.Tab>
    ```bash
    pnpm exec ts-node src/quickstart.ts
    ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```bash
