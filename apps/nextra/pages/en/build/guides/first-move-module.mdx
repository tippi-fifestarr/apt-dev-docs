...
<Callout type="info">
The Aptos CLI is also required for compiling, testing, and publishing Move modules.
</Callout>
...
## Explore Your Newly Funded Account On-Chain
...
## Writing and Compiling Your First Module
...
This module stores a message on-chain in a resource, then retrieves it whenever someone calls `get_message`.
...
module my_first_module::message {
    ...
    // A resource that stores a string message under a user's account
    struct MessageHolder has key, store, drop {
        message: string::String,
    }

    // Allows an account to store a message in their account storage
    // The 'acquires' keyword indicates this function accesses the MessageHolder resource
    public entry fun set_message(account: &signer, message: string::String) acquires MessageHolder {
        let account_addr = signer::address_of(account);

        // If a message already exists, remove it first
        if (exists<MessageHolder>(account_addr)) {
            move_from<MessageHolder>(account_addr);
        };

        // Store the new message under the user's account
        // 'move_to' is a critical operation that places resources in account storage
        move_to(account, MessageHolder { message });
    }

    // Retrieves a message stored in an account's storage
    public fun get_message(account_addr: address): string::String acquires MessageHolder {
        ...
        // Borrow a reference to the stored message
...
2. [Ability modifiers](../smart-contracts/book/abilities) like `key`, `store`, and `drop` control how values can be used
