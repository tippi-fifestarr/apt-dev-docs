...
1. Create an empty Python script:
    
```bash filename="Terminal"
touch multisig.py
```
    
2. Open `multisig.py` in your text editor and add the following code:
...
<Steps>
### Generate Key Holder Accounts
1. Add the following code after `############# Add additional code here ###############`:
...
2. Add this code below `chad = Account.generate()`:
...
3. Add the following code to print out the rest of the public account information:
...
### Run the Script
4. Run the code to see three newly generated accounts:
    
```bash filename="Terminal"
python3 multisig.py
```
    
5. Press enter after seeing the new account numbers to see all three pieces of information.
...
<Callout type="info">
For each user, note the [account address](../concepts/accounts#account-address) and [authentication key](../concepts/accounts#authentication-key) are identical, but the [public key](../concepts/accounts#creating-an-account) is different.
...
2. When we run it, there will be a slight pause here as it waits for the funding to go through. Below the `await` add this code to check all balances:
...
### Verify Funding
3. Run the code with `python3 multisig.py` to verify funding success:
...
<Steps>
### Combine Signatures
1. Combine the signatures into a multisig authenticator:
...
### Run Final Script
4. Run our script again with `python3 multisig.py` to get our transaction hash and multisig address for our successfully submitted transaction!
...
2. Learn about [multisig governance in this tutorial](../guides/multi-signature-tutorial)
3. Explore [account abstraction in Aptos](../concepts/accounts)
